---
name: Zoho Meeting
authority: zoho
market: Global
type: platform
status: active
enacted: 2011-01-01
effective: 2011-01-01
official_url: https://zoho.com/meeting/
pricing_page: https://zoho.com/meeting/pricing.html
range: 2-250
last_verified: 2026-09-05
---

## Timeline

| Milestone | Date | Notes |
|-----------|------|-------|
| Launch | 2011-01-01 | Online meeting and webinar platform as part of Zoho suite |
| Zoho Meeting 2.0 | 2020-06-01 | Major redesign with improved video and collaboration features |
| Breakout rooms added | 2022-01-01 | Support for breakout sessions in meetings |

## Pricing

| Plan | Price | Notes |
|------|-------|-------|
| Free | $0/mo | 60-min meetings, 100 participants |
| Standard | Capacity-based | Price varies by participant tier and host count |
| Professional | Capacity-based | Price varies by participant tier and host count; includes recording |

---

## Video Conferencing

| Property | Value |
|----------|-------|
| Obligation | video-conferencing |
| Sections | Meeting |
| Status | active |
| Effective | 2011-01-01 |
| Verified | 2026-09-05 |
| Checked | 2026-09-05 |

### Requirements

| Requirement | Details |
|-------------|---------|
| Participant limit | Up to 250 participants depending on plan |
| Video quality | HD video with adaptive streaming |
| Layout options | Speaker view and grid view |

### Editorial Summary

Zoho Meeting offers affordable video conferencing tightly integrated with the broader Zoho productivity ecosystem.

### Sources

- [Zoho Meeting](https://zoho.com/meeting/)

---

## Screen Sharing

| Property | Value |
|----------|-------|
| Obligation | screen-sharing |
| Sections | Meeting |
| Status | active |
| Effective | 2011-01-01 |
| Verified | 2026-09-05 |
| Checked | 2026-09-05 |

### Requirements

| Requirement | Details |
|-------------|---------|
| Sharing options | Full screen, application window, or browser tab |
| Annotation | Presenter can annotate shared screen |
| Remote control | Participants can request remote control |

### Editorial Summary

Zoho Meeting supports screen sharing with annotation tools for interactive presentations.

### Sources

- [Zoho Meeting](https://zoho.com/meeting/)

---

## Breakout Rooms

| Property | Value |
|----------|-------|
| Obligation | breakout-rooms |
| Sections | Meeting |
| Status | active |
| Effective | 2022-01-01 |
| Verified | 2026-09-05 |
| Checked | 2026-09-05 |

### Requirements

| Requirement | Details |
|-------------|---------|
| Room creation | Host can create multiple breakout rooms |
| Assignment | Manual or automatic participant assignment |
| Broadcast | Host can broadcast messages to all rooms |

### Editorial Summary

Zoho Meeting added breakout rooms to support collaborative small-group sessions within larger meetings.

### Sources

- [Zoho Meeting](https://zoho.com/meeting/)

---

## Recording and Transcription

| Property | Value |
|----------|-------|
| Obligation | recording |
| Sections | Meeting |
| Status | active |
| Effective | 2011-01-01 |
| Verified | 2026-09-05 |
| Checked | 2026-09-05 |

### Requirements

| Requirement | Details |
|-------------|---------|
| Cloud recording | Meetings recorded and stored in the cloud |
| Local recording | Option to download recordings locally |
| Playback | Browser-based playback of recorded sessions |

### Editorial Summary

Zoho Meeting provides cloud recording with easy sharing and playback within the Zoho ecosystem.

### Sources

- [Zoho Meeting](https://zoho.com/meeting/)

---

## Chat and Messaging

| Property | Value |
|----------|-------|
| Obligation | chat-messaging |
| Sections | Meeting, Zoho Cliq |
| Status | active |
| Effective | 2011-01-01 |
| Verified | 2026-09-05 |
| Checked | 2026-09-05 |

### Requirements

| Requirement | Details |
|-------------|---------|
| In-meeting chat | Text chat during live meetings |
| File sharing | Share files through in-meeting chat |
| Zoho Cliq | Persistent team messaging via Zoho Cliq integration |

### Editorial Summary

In-meeting chat integrates with Zoho Cliq for seamless team communication across the Zoho suite.

### Sources

- [Zoho Meeting](https://zoho.com/meeting/)

---

## Polling and Q&A

| Property | Value |
|----------|-------|
| Obligation | polling |
| Sections | Meeting, Webinar |
| Status | active |
| Effective | 2011-01-01 |
| Verified | 2026-09-05 |
| Checked | 2026-09-05 |

### Requirements

| Requirement | Details |
|-------------|---------|
| Polls | Create and launch polls during meetings and webinars |
| Question types | Multiple choice and open-ended questions |
| Results | Real-time results displayed to host |

### Editorial Summary

Zoho Meeting's polling features are particularly useful for webinar engagement and audience feedback.

### Sources

- [Zoho Meeting](https://zoho.com/meeting/)

---

## Whiteboard

| Property | Value |
|----------|-------|
| Obligation | whiteboard |
| Sections | Meeting |
| Status | active |
| Effective | 2022-01-01 |
| Verified | 2026-09-05 |
| Checked | 2026-09-05 |

### Requirements

| Requirement | Details |
|-------------|---------|
| Drawing tools | Pen, shapes, and text tools |
| Collaboration | Multiple participants can draw simultaneously |
| Export | Whiteboard can be saved and exported |

### Editorial Summary

Zoho Meeting's whiteboard provides a collaborative visual workspace integrated directly into the meeting experience.

### Sources

- [Zoho Meeting](https://zoho.com/meeting/)

---

## Virtual Backgrounds

| Property | Value |
|----------|-------|
| Obligation | virtual-backgrounds |
| Sections | Meeting |
| Status | active |
| Effective | 2020-06-01 |
| Verified | 2026-09-05 |
| Checked | 2026-09-05 |

### Requirements

| Requirement | Details |
|-------------|---------|
| Background images | Choose from built-in library or upload custom images |
| Background blur | Blur background to reduce distractions |
| Preview | Preview background before joining meeting |

### Editorial Summary

Zoho Meeting offers virtual backgrounds and blur options to maintain professionalism in any setting.

### Sources

- [Zoho Meeting](https://zoho.com/meeting/)
