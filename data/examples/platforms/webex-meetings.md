---
name: Webex Meetings
authority: cisco
market: Global
type: platform
status: active
enacted: 2007-05-01
effective: 2007-05-01
official_url: https://webex.com
pricing_page: https://webex.com/pricing.html
range: 2-1,000
last_verified: 2026-09-05
---

## Timeline

| Milestone | Date | Notes |
|-----------|------|-------|
| WebEx founded | 1995-01-01 | Originally founded as ActiveTouch |
| Cisco acquisition | 2007-05-01 | Cisco acquires WebEx Communications for $3.2B |
| Webex Teams launch | 2018-04-01 | Persistent messaging and collaboration added |
| Webex Suite rebrand | 2021-03-01 | Unified Webex branding across meetings, messaging, and calling |
| AI Assistant | 2023-12-01 | Webex AI Assistant for meeting summaries and real-time translation |

## Pricing

| Plan | Price | Notes |
|------|-------|-------|
| Free | Verify current offer | Plan details vary by market |
| Paid plans | Quote or market-specific | Verify current price, capacity, and entitlements directly with Webex |

---

## Video Conferencing

| Property | Value |
|----------|-------|
| Obligation | video-conferencing |
| Sections | Meetings |
| Status | active |
| Effective | 2007-05-01 |
| Verified | 2026-09-05 |
| Checked | 2026-09-05 |

### Requirements

| Requirement | Details |
|-------------|---------|
| Grid view | Up to 81 participants visible in grid layout |
| HD video | 720p and 1080p support |
| Noise removal | AI-powered background noise suppression |

### Editorial Summary

Webex supports up to 1,000 participants per meeting and 81 on-screen in grid view on enterprise plans.

### Sources

- [Webex Meetings](https://webex.com/video-conferencing)

---

## Screen Sharing

| Property | Value |
|----------|-------|
| Obligation | screen-sharing |
| Sections | Meetings |
| Status | active |
| Effective | 2007-05-01 |
| Verified | 2026-09-05 |
| Checked | 2026-09-05 |

### Requirements

| Requirement | Details |
|-------------|---------|
| Desktop sharing | Full screen, application window, or browser tab |
| Annotation | Drawing and annotation tools on shared content |
| Remote control | Host can grant remote desktop control to participants |

### Editorial Summary

Webex screen sharing includes built-in annotation tools and remote control for technical support scenarios.

### Sources

- [Webex Meetings](https://webex.com/video-conferencing)

---

## Breakout Rooms

| Property | Value |
|----------|-------|
| Obligation | breakout-rooms |
| Sections | Meetings |
| Status | active |
| Effective | 2020-09-01 |
| Verified | 2026-09-05 |
| Checked | 2026-09-05 |

### Requirements

| Requirement | Details |
|-------------|---------|
| Room count | Up to 100 breakout sessions per meeting |
| Assignment | Pre-assign or auto-assign participants |
| Controls | Broadcast messages, set timers, and move between rooms |

### Editorial Summary

Webex supports up to 100 breakout sessions with pre-assignment capabilities for large training events.

### Sources

- [Webex Breakout Sessions](https://help.webex.com/en-us/article/ncp94mq/Breakout-sessions)

---

## Recording and Transcription

| Property | Value |
|----------|-------|
| Obligation | recording |
| Sections | Meetings |
| Status | active |
| Effective | 2007-05-01 |
| Verified | 2026-09-05 |
| Checked | 2026-09-05 |

### Requirements

| Requirement | Details |
|-------------|---------|
| Cloud recording | MP4 recording stored in Webex cloud |
| Local recording | Available for desktop client users |
| Transcription | Automated meeting transcripts with speaker attribution |

### Editorial Summary

Webex cloud recordings include automated transcription and AI-generated meeting highlights.

### Sources

- [Webex Recording](https://webex.com/video-conferencing)

---

## Chat and Messaging

| Property | Value |
|----------|-------|
| Obligation | chat-messaging |
| Sections | Messaging |
| Status | active |
| Effective | 2007-05-01 |
| Verified | 2026-09-05 |
| Checked | 2026-09-05 |

### Requirements

| Requirement | Details |
|-------------|---------|
| In-meeting chat | Public and private messages during meetings |
| Persistent messaging | Spaces with threaded conversations and file sharing |
| Integrations | Connects to third-party tools via app hub |

### Editorial Summary

Webex Messaging provides persistent spaces that carry context before and after meetings.

### Sources

- [Webex Messaging](https://webex.com/team-collaboration)

---

## Polling

| Property | Value |
|----------|-------|
| Obligation | polling |
| Sections | Meetings, Webinars |
| Status | active |
| Effective | 2021-10-01 |
| Verified | 2026-09-05 |
| Checked | 2026-09-05 |

### Requirements

| Requirement | Details |
|-------------|---------|
| Slido integration | Native Slido polling embedded in Webex |
| Poll types | Multiple choice, word cloud, rating, open text, and quizzes |
| Analytics | Real-time results and post-meeting polling reports |

### Editorial Summary

Webex integrates Slido natively, providing advanced polling including word clouds and quizzes without a separate app.

### Sources

- [Webex Slido Integration](https://webex.com/slido)

---

## Whiteboard

| Property | Value |
|----------|-------|
| Obligation | whiteboard |
| Sections | Whiteboarding |
| Status | active |
| Effective | 2021-03-01 |
| Verified | 2026-09-05 |
| Checked | 2026-09-05 |

### Requirements

| Requirement | Details |
|-------------|---------|
| Collaboration | Real-time multi-user digital canvas |
| Templates | Pre-built templates for brainstorming and workflows |
| Persistence | Whiteboards saved and accessible outside meetings |

### Editorial Summary

Webex Whiteboard offers a persistent collaborative canvas with templates, usable in and outside meetings.

### Sources

- [Webex Whiteboard](https://webex.com/whiteboard)

---

## Virtual Backgrounds

| Property | Value |
|----------|-------|
| Obligation | virtual-backgrounds |
| Sections | Meetings |
| Status | active |
| Effective | 2020-06-01 |
| Verified | 2026-09-05 |
| Checked | 2026-09-05 |

### Requirements

| Requirement | Details |
|-------------|---------|
| Static backgrounds | Built-in library and custom image upload |
| Background blur | Adjustable blur effect |
| Immersive sharing | Place presenter video over shared content |

### Editorial Summary

Webex virtual backgrounds include immersive share mode, placing the presenter directly into the presentation.

### Sources

- [Webex Virtual Backgrounds](https://help.webex.com/en-us/article/0p4gb1/Use-a-virtual-or-blurred-background)

---

## Integrated AI Note-Taker

| Property | Value |
|----------|-------|
| Obligation | ai-notetaker |
| Sections | AI Assistant |
| Status | active |
| Effective | 2024-03-01 |
| Verified | 2026-09-05 |
| Checked | 2026-09-05 |

### Requirements

| Requirement | Details |
|-------------|---------|
| Meeting summary | AI-generated post-meeting summary with key decisions |
| Real-time suggestions | In-meeting AI suggestions for action items and follow-ups |

### Editorial Summary

Webex AI Assistant generates real-time meeting summaries and can catch up latecomers with contextual recaps.

### Sources

- [Webex Meetings](https://webex.com)
